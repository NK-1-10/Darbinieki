...
app.get('/api/workers', async (req, res) => {
    try {
        const result = await pool.query("SELECT name, temp_password, role FROM users WHERE role != 'admin' OR role IS NULL ORDER BY name ASC");
        res.json(result.rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.delete('/api/cars/:name', async (req, res) => {
    const { name } = req.params;
    try {
        await pool.query('DELETE FROM cars WHERE name = $1', [name]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.delete('/api/objects/:name', async (req, res) => {
    const { name } = req.params;
    try {
        await pool.query('DELETE FROM objects WHERE name = $1', [name]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/work-types', async (req, res) => {
    const { name } = req.body;
    try {
        await pool.query('INSERT INTO work_types (name) VALUES ($1)', [name]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.delete('/api/work-types/:name', async (req, res) => {
    const { name } = req.params;
    try {
        await pool.query('DELETE FROM work_types WHERE name = $1', [name]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/stop-work', async (req, res) => {
    ...
    try {
        ...
        if (activeJob.rows.length > 0) {
            const start = activeJob.rows[0].sākuma_laiks;
            const [sh, sm, ss] = start.split(':').map(Number);
            const [eh, em, es] = timeOnly.split(':').map(Number);
            let diffSeconds = (eh * 3600 + em * 60 + es) - (sh * 3600 + sm * 60 + ss);
            if (diffSeconds < 0) diffSeconds += 86400;
            const hoursStr = (diffSeconds / 3600).toFixed(2);
            ...
            res.status(404).json({ error: "Nav aktīva darba šim darbiniekam" });
        }
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/update-resources', async (req, res) => {
    ...
    const months = ["Janvāris", "Februāris", "Marts", "Aprīlis", "Maijs", "Jūnijs", "Jūlijs", "Augusts", "Septembris", "Oktobris", "Novembris", "Decembris"];
    const mēnesis = months[tagad.getMonth()];
    ...
    try {
        ...
        if (activeJob.rows.length > 0) {
            ...
            await pool.query(
                ...
                [worker_name, car, datums, laiks, mēnesis, litri.toString(), type === 'Ella' ? 'Eļļas papildināšana' : 'Degvielas uzpilde']
            );
        }
        res.sendStatus(200);
    } catch (err) {
